...
def main(task_list_path, gcalcli_import=False, start_time_str=None):
    # Load configuration
    config = load_config()
    ...
    task_list = read_task_list(args.task_list_path)
    ...
    create_ics_files(scheduled_tasks, calendar_mapping, args.task_list_path)

    # import .ics files to Google calendar with gcalcli
    if args.gcalcli_import:
        import_to_gcalcli(calendar_mapping, args.task_list_path)
...
def cli_wrapper():
    ...
    main(args.task_list_path, args.gcalcli_import, args.start_time)
